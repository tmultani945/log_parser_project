...
def find_table_caption(page_text: str, page_num: int) -> str:
    """Find table caption from page text (legacy - returns first caption)"""
    captions = find_all_table_captions(page_text, page_num)
    return captions[0] if captions else f"Table on page {page_num + 1}"
    ...
    if pattern.search(text):
        ...
        if tables and len(tables) > 0:
            ...
            if rows:
                ...
                # Try to get full caption from page text
                caption_match = re.search(rf"Table\s+{re.escape(table_number)}[:\s]*(.+?)(?:\n|$)", text, re.IGNORECASE)
                if caption_match:
                    caption = f"Table {table_number}: {caption_match.group(1).strip()}"
...
def export_to_json(
    ...
    # Filter version_map to only include target version
    filtered_version_map = {str(TARGET_VERSION): version_map.get(TARGET_VERSION, "")} if TARGET_VERSION in version_map else {}
    ...
    metadata = {
        ...
        "version_map": filtered_version_map,
